...
# Loading raw data
An overview of the data is given in the BExIS file 27088 (Additional information). 

In order to read in data correctly, the path to the folder which contains the data needs to be given (as variable `pathtodata`). "The data" refers to the downloaded and unzipped data files as they are downloaded from Bexis. The names of the BExIS files are given in 27088 "zipfile name", in order to confirm versions. All extracted zip files should be stored together in the same folder (where `pathtodata` points to).
...
pathtodata is pasted together with the name of the zip file, which is stored in the file "27088_helper.csv". "27088_helper.csv" is stored together with the code, as well as under "helper_data".
TODO : the version in "helper_data" will be removed.

Reading pathtodata and the file "27088_helper.csv"
```{r}
...
#USER : insert path to 27088_helper.csv
# info_data <- data.table::fread("/run/user/1000/gvfs/smb-share:server=nas-ips,share=ips/groups/planteco/PROJECTS/Exploratories Synthesis/Research Projects/27087_synthesis_grassland_functions_construction/helper_data/27088_helper.csv")
info_data <- data.table::fread("27088_helper.csv")
```
...
# 2009
Biomass2009 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2009", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2010
Biomass2010 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2010", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2011
Biomass2011 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2011", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2012
Biomass2012 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2012", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2013
Biomass2013 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2013", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2014
Biomass2014 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2014", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2015
Biomass2015 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2015", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t', quote="", drop=19)
...
# 2016
Biomass2016 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2016", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
# 2017
Biomass2017 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Biomass2017", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
**soil C concentration** : C in soil (former name : soil C stock)
...
```{r}
#2011
Bulk.density.soil.C.stock.2011 <- data.table::fread(paste(pathtodata, info_data$subpath_unix[grep("Soil.C.concentration", info_data$AggregatedColumnName)], sep=""), header=T, sep='\t')
...
```{r}
path <- paste(pathtodata, info_data$subpath_unix[grep("pathogen.infection", info_data$AggregatedColumnName)], sep="")
