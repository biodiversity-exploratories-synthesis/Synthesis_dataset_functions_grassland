...
```{r}
N_leaching_risk2015 <- data.table::fread(paste(pathtodata, info_data$subpath[grep("N_leaching_risk2015", info_data$ColumnName)], sep=""), header=T, sep='\t')
...
[µg/g Resin] 	
```{r}
P_leaching_risk2015 <- data.table::fread(paste(pathtodata, info_data$subpath[grep("P_leaching_risk2015", info_data$ColumnName)], sep=""), header=T, sep='\t')
...
```{r}
dung.removal <- data.table::fread(paste(pathtodata, info_data$subpath[grep("dung.removal$", info_data$ColumnName)], sep=""), header=T, sep='\t')
...
134 plots
```{r}
seed <- data.table::fread(paste(pathtodata, info_data$subpath[grep("seed.depletion", info_data$ColumnName)], sep=""), header=T, sep='\t')
...
```{r, eval = F}
pred <- data.table::fread(paste(pathtodata, info_data$subpath[grep("caterpillars.predation", info_data$ColumnName)], sep=""), header=T, sep='\t')
pred <- pred[grep("G", pred$PlotID),]
pred <- aggregate(Predation_prop ~ PlotID, pred, function(x) mean(x, na.rm=T))
#TODO HERE finish loading as for other functions and move calculation to 2calc.
```
