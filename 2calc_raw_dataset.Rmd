...
```{r, eval=T}
# 2011 / 2009
rPRI <- raw_functions[, c("Plot", "Pmic.2011", "Bulk.density2011", "NaHCO3_Pi", "Biomass2011")]
nutrients <- list_raw_functions$nutrients
nutrients <- nutrients[, c("Plot", "Pshoot.2011")]
rPRI <- merge(rPRI, nutrients, by = "Plot", all = T)

rPRI[, "micPstock" := (Pmic.2011 / 1000000)*(Bulk.density2011 * 1000000)]
rPRI[, "soilPstock" := (NaHCO3_Pi / 1000000) * (Bulk.density2011 * 1000000)]
...
## Tests for Mini-Multifunc
The below code contains considerations about the grouping of the mini-multifunctionalities. It appears that some of them do not correlate with each other as expected. 
```{r, eval=F}
source("input_for_2calc.R")
# contains correlation plots, correlation networks and a pca
```
...
```{r}
nce <- raw_functions[,c("Plotn", "Urease", "DEA","Potential.nitrification2011", "Potential.nitrification2014","nifH","amoA_AOB.2011","amoA_AOA.2011", "amoA_AOB.2016", "amoA_AOA.2016", "nxrA_NS", "16S_NB")]
...
```{r}
raw_functions[, "soilAmmoniaflxs" := multidiv(nce_a[, !"Plotn", with=F], sc="sd", cent=T)[,1]]
raw_functions[, "soilNitrateflxs" := multidiv(nce_n[, !"Plotn", with=F], sc="sd", cent=T)[,1]]

rm(nce_n); rm(nce_a); rm(nce)
raw_functions[,c("Urease", "DEA","Potential.nitrification2011", "Potential.nitrification2014","nifH","amoA_AOB.2011","amoA_AOA.2011", "amoA_AOB.2016", "amoA_AOA.2016", "nxrA_NS", "16S_NB") := NULL]
```

## P_loss_comb
combination of PRI and P_loss/ P_leaching risk.

PRI was measured in 2014 and is not correlated with the P_loss measures. Therefore, this is not calculated here.
```{r, eval=F}
plc <- raw_functions[, c("P_loss2011", "P_leaching_risk2015", "PRI", "PRI.2011")]
# take mean of P_loss 2011 and leaching risk 2015
plc[, "P_loss" := apply(plc[,c("P_loss2011", "P_leaching_risk2015")],1, mean)]
plc[, "PRI1" := apply(plc[, c("PRI", "PRI.2011")], 1, mean)]
plc[, c("P_loss2011", "P_leaching_risk2015", "PRI", "PRI.2011") := NULL]
